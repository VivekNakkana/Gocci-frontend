...
const Cart = () => {
  ...
  const payment = async (token) => {
    await axios.post("http://localhost:6000/pay", {
      amount: totalAmt * 100,
      token: token,
    });
  };
          ...
          <div className="w-1/3 bg-[#fafafa] py-6 px-4">
            ...
            {payNow && (
              <div className="w-full mt-6 flex flex-col items-center justify-center">
                <StripeCheckout
                  stripeKey="pk_test_51MoftBSCotuGlagHxhphOf08KTnemmORdOgVVZomrS3TIMP2cPehgOuxydIBOgQRTRaS7umgcO7yGdTUtM8W1o1K0027rZcsEV"
                  name="Bazar Online Shopping"
                  amount={totalAmt * 100}
                  label="Pay using Strpie"
                  description={`Your Payment amount is $${totalAmt}`}
                  token={payment}
                  email={userInfo.email}
                />
                <p>For test purpose use</p> 
                <p>Card: 424242424242 date:12/34 cvv:123
                </p>
              </div>
            )}
          </div>
